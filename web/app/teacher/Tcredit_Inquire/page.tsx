...
export default function App() {
  ...
  const refreshUsers = () => {
    if (process.env.NEXT_PUBLIC_TEST === "test") {
      ...
      try {
        ...
        if (userInfo)
          API.TeacherServiceApi.getTcredit(userInfo.userId)
            .then((res) => {
              console.log(res)
              if (res.status === 200) {
                if (res.data.Tcredit) setCredits(res.data.Tcredit);
              }
              else {
                error("获取教分情况失败！");
              }
            ...
            .catch((err: any) => {
              console.log("Get Tcredit Error: ", err);
              error("Get Tcredit Error: " + err);
            });
      } catch (err: any) {
        console.log("Get Tcredit Error: ", err);
        error("Get Tcredit Error: " + err);
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-20 justify-center items-center px-28">
      <div className="w-full text-start text-3xl font-bold">教分完成情况</div>
      <Table
