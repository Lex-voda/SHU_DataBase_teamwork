...
**上海大学2023春季数据库2课程作业代码仓库**

**联合作者：**SHU_2021： lsh，tjc，mwj，zxy
#### 一键体验
双击 run.bat
#### 杂项写前面

日志文件名格式：与请求头同时创建，请求头不变，文件名不变（后来没来得及做日志系统）
...
2024/5/13:	所有的flag都改为 True/Fasle
			修改了我的预约查询的通信字典
			所有的路由都改为POST（后来证明其实是前端问题，不是GET问题）
...
- 格式检查断言：任何对表的注入都需要满足格式要求，需要进行断言检查，尽管大部分表没有开放给用户的注入接口，但是也要设计对应的格式检查断言（报告内容+++）（其实后来没写到库里）
